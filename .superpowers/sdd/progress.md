...
Final review (opus whole-branch): Ready after fixes. Fixed 1 Important cross-org isolation gap: assign-bulk now ties target node to each user's org (commit 638f45b, +regression test). Deferred Minors accepted (O(n) scan documented hot spot; move wrong-type parent 404; assign-bulk missing-node 403). Branch structural-ops ready to merge.
